...
func (h *Host) writeFrame(v interface{}) error {
	...
	if err := binary.Write(h.w, binary.LittleEndian, uint32(len(body))); err != nil {
...
func (h *Host) readFrame(dst interface{}) error {
	var length uint32
...
func (h *Host) Send(msg NativeMessage) error      { return h.writeFrame(msg) }
func (h *Host) SendRaw(v interface{}) error        { return h.writeFrame(v) }
func (h *Host) Receive(resp *NativeResponse) error { return h.readFrame(resp) }
...
func (h *Host) ReceiveRequest(msg *NativeMessage) error { return h.readFrame(msg) }
